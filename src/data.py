...
class DiscogsPageBase:
    def __init__(self, url, session=None, proxies=None, max_requests_per_minute=25):
        self.url = url
    ...
    def fetch_page_content(self):
        """Fetches page content with rate limiting using the session."""
        logging.debug(f"Fetching page content for URL: {self.url} using session: {self.session} in thread: {threading.current_thread().name}")
        try:
...
class DiscogsRelease(DiscogsPageBase):
    def __init__(self, release_id, session=None, proxies=None, max_requests_per_minute=25):
        self.release_id = release_id
        self.url = f'https://www.discogs.com/release/{release_id}'
        super().__init__(self.url, session, proxies, max_requests_per_minute)
        self.stats = None
...
class DiscogsStatsPage(DiscogsPageBase):
    def __init__(self, release_id, session=None, proxies=None, max_requests_per_minute=25):
        self.release_id = release_id
        self.url = f"https://www.discogs.com/release/stats/{release_id}"
        super().__init__(self.url, session, proxies, max_requests_per_minute)
...
class DiscogsSellerPageBase(DiscogsPageBase):
    def __init__(self, base_url, session=None, proxies=None, query_params=None, max_requests_per_minute=25):
        if query_params:
            query_string = urlencode(query_params)
            url = f"{base_url}?{query_string}"
        else:
            url = base_url
        super().__init__(url, session, proxies, max_requests_per_minute)
        self.items_for_sale = []
...
class DiscogsSellerPage(DiscogsSellerPageBase):
    def __init__(self, username, session=None, proxies=None, query_params=None, max_requests_per_minute=25):
        base_url = f"https://www.discogs.com/seller/{username}/profile"
        super().__init__(base_url, session, proxies, query_params, max_requests_per_minute)

class DiscogsSellerPageRelease(DiscogsSellerPageBase):
    def __init__(self, release_id, session=None, proxies=None, query_params=None, max_requests_per_minute=25):
